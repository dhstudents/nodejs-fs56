...
app.get('/employee/:deprtment/:employeeId', (req, res) => {
    ...
    const { deprtment , employeeId} = req.params
    res.send(`<h1>[Rest Result <u style="color: red">req.params</u>] DEPR : ${deprtment} EMPID : ${employeeId}`)
})

app.get('/employee/' , (req, res) => {
    console.log("QUERY", req.query)
    const { deprtment , employeeId} = req.query
    res.send(`<h1>[Query Result <u style="color: red">req.query</u>] DEPR : ${deprtment} EMPID : ${employeeId}`)
})
